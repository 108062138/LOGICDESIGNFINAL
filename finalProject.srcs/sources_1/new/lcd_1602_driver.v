...
input wire rst_n,
output wire lcd_en,
output wire lcd_rw,  //因为只执行写操作，所以永远为0.
output reg lcd_rs,
...
always  @(posedge clk or negedge rst_n)begin
    ...
    end else if(write_flag == 1)begin
        if(( c_state==SET_FUNCTION)||(c_state==DISP_OFF)||
            (c_state==DISP_CLEAR)||(c_state==ENTRY_MODE)||
            (c_state==DISP_ON ) ||(c_state==ROW1_ADDR)||
            (c_state==ROW2_ADDR))begin
            lcd_rs<=0 ;
        end else begin
            lcd_rs<= 1;
        end
